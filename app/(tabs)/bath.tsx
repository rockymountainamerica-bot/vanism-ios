...
import { getActivePlanBathData, setBathDay, ActivePlanBathData, PlanBathSpotRow } from '@/lib/db';
...
export default function BathScreen() {
  const [data, setData]     = useState<ActivePlanBathData | null | undefined>(undefined);
  const [distMi, setDistMi] = useState<number | null>(null);

  const load = useCallback(() => {
    const d = getActivePlanBathData();
    setData(d);
    setDistMi(null);
    if (!d) return;
  ...
  if (!data || data.spots.length === 0) {
    return (
      <View style={styles.root}>
        <Text style={styles.emptyText}>No bath spots yet — load a plan first.</Text>
      </View>
    );
  }
  ...
  function handleAdvance() {
    ...
    setBathDay(data.planId, data.currentDay + 1);
    load();
  }

  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      <Text style={styles.header}>BATH · Stop {data.currentDay} of {maxDay}</Text>
      ...
      {data.spots.map(spot => {
        ...
        const inner = (
          <SpotRow
            spot={spot}
            spotState={spotState}
            label={`Bath Stop ${spot.day_number}`}
      ...
      {data.currentDay <= maxDay && (
        <Text style={styles.hint}>Swipe right on today's bath stop when you're done</Text>
      )}
    </ScrollView>
  );
}
